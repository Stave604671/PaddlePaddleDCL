...
class dataset(paddle.io.Dataset):
    def __init__(self, Config, anno, swap_size=[7, 7], common_aug=None, swap=None, totensor=None, train=False,
        ...
        if isinstance(anno, pandas.DataFrame):
    ...
    def __getitem__(self, item):
        ...
        if self.train:
            ...
            image_swap_list = [Image.fromarray(im) for im in image_swap_list]
            image_unswap_list = [Image.fromarray(im) for im in image_unswap_list]
    ...
    def cv2_loader(self, imgpath):
        return cv2.imread(imgpath)
    def crop_image(self, image, cropnum):
        width, high, ch = image.shape
        crop_x = [int((width / cropnum[0]) * i) for i in range(cropnum[0] + 1)]
        crop_y = [int((high / cropnum[1]) * i) for i in range(cropnum[1] + 1)]
        im_list = []
        for j in range(len(crop_y) - 1):
            for i in range(len(crop_x) - 1):
                im_list.append(paddle.vision.transforms.crop(image, crop_x[i], crop_y[j], min(crop_x[i + 1], width), min(crop_y[j + 1], high)))
        return im_list

    def get_weighted_sampler(self):
        img_nums = len(self.labels)
        weights = [self.labels.count(x) for x in range(self.numcls)]
        return paddle.io.WeightedRandomSampler(weights, num_samples=img_nums)
