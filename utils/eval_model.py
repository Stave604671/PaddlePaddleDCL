...
def eval_turn(Config, model, data_loader, val_version, epoch_num, log_file):
    ...
    with paddle.no_grad():
        for batch_cnt_val, data_val in enumerate(data_loader):
            inputs = data_val[0]
            labels = paddle.to_tensor(np.array(data_val[1])).floor()
            ...
            batch_corrects1 = paddle.sum((top3_pos[:, 0] == labels)).item()
            val_corrects1 += batch_corrects1
            batch_corrects2 = paddle.sum((top3_pos[:, 1] == labels)).item()
            val_corrects2 += (batch_corrects2 + batch_corrects1)
            batch_corrects3 = paddle.sum((top3_pos[:, 2] == labels)).item()
        ...
        log_file.write(val_version + '\t' + str(val_loss_recorder.get_val()) + '\t' + str(
            val_celoss_recorder.get_val()) + '\t' + str(val_acc1) + '\t' + str(val_acc3) + '\n')
