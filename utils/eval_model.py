...
def eval_turn(Config, model, data_loader, val_version, epoch_num, log_file):
    ...
    for batch_cnt_val, data_val in enumerate(data_loader):
        ...
        labels = paddle.to_tensor(np.array(data_val[1]), dtype="int64")
                # .floor()
        outputs = model(inputs)
        loss = 0
        ce_loss = get_ce_loss(outputs[0], labels)
        ce_loss = ce_loss.numpy()[0]
        loss += ce_loss

        val_loss_recorder.update(loss)
        val_celoss_recorder.update(float(ce_loss))

        if Config.use_dcl and Config.cls_2xmul:
            outputs_pred = outputs[0] + outputs[1][:, 0:num_cls] + outputs[1][:, num_cls:2 * num_cls]
        else:
            outputs_pred = outputs[0]
        top3_val, top3_pos = paddle.topk(outputs_pred, 3)

        print('{:s} eval_batch: {:-6d} / {:d} loss: {:8.4f}'.format(val_version, batch_cnt_val, val_epoch_step, loss))

        batch_corrects1 = paddle.sum((top3_pos[:, 0] == labels))
        batch_corrects1 = batch_corrects1.numpy()[0]
        val_corrects1 += batch_corrects1
        batch_corrects2 = paddle.sum((top3_pos[:, 1] == labels))
        batch_corrects2 = batch_corrects2.numpy()[0]
        val_corrects2 += (batch_corrects2 + batch_corrects1)
        batch_corrects3 = paddle.sum((top3_pos[:, 2] == labels))
        batch_corrects3 = batch_corrects3.numpy()[0]
    ...
    since = t1 - t0
    print('--' * 30, flush=True)
    print('% 3d %s %s %s-loss: %.4f ||%s-acc@1: %.4f %s-acc@2: %.4f %s-acc@3: %.4f ||time: %d' % (
        epoch_num, val_version, dt(), val_version, val_loss_recorder.get_val(init=True), val_version, val_acc1,
        val_version, val_acc2, val_version, val_acc3, since), flush=True)
    print('--' * 30, flush=True)
