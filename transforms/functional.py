...
def swap(img, crop):
    def crop_image(image, cropnum):
        ...
        for j in range(len(crop_y) - 1):
            for i in range(len(crop_x) - 1):
                im_list.append(paddle.vision.transforms.crop(image,
                                                             crop_x[i],
                                                             crop_y[j],
    ...
    widthcut, highcut, ch = img.shape
    img = paddle.vision.transforms.crop(img, 10, 10, widthcut-10, highcut-10)
    ...
    if pro >= 5:          
        ...
        y = 0
        for i in random_im:
            i = paddle.vision.transforms.resize(i, (iw, ih))# , Image.ANTIALIAS)
            PIL_Image = Image.fromarray(i)
            toImage.paste(PIL_Image, (x * iw, y * ih))
            x += 1
            if x == crop[0]:
                x = 0
                y += 1
