...
if __name__ == '__main__':
    ...
    val_set = dataset(Config=Config,
                      ...
                      test=True)
    dataloader = {}
    dataloader['train'] = paddle.io.DataLoader(train_set,
    ...
    dataloader['trainval'] = paddle.io.DataLoader(trainval_set,
    ...
    dataloader['val'] = paddle.io.DataLoader(val_set,
                                                    batch_size=args.val_batch,
                                                    shuffle=False,
                                                    num_workers=args.val_num_workers,
    ...
    if (args.resume is None) and (not args.auto_resume):
        ...
        pretrained_dict = paddle.load(resume)
    ...
    exp_lr_scheduler = optim.lr.StepDecay(learning_rate=base_lr, step_size=args.decay_step, gamma=0.1)
    if Config.use_backbone:
        optimizer = optim.Momentum(learning_rate=exp_lr_scheduler, parameters=[{'params': base_params},
                               {'params': model.classifier.parameters(), 'lr': base_lr}],
                              momentum=0.9)
    else:
        # model_param = paddle.nn.LayerList([base_params])
        optimizer = optim.Momentum(learning_rate=exp_lr_scheduler, parameters=model.parameters(), momentum=0.9)
        #[{'params': base_params},
